...
export default function JobDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function loadJobDetails() {
      try {
        setLoading(true);
        const data = await apiFetch(`/production/jobs/${id}`);
        setJob(data.job);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default: return "bg-zinc-500 border-zinc-500/20";
    }
  };
    ...
    return (
      <DashboardLayout>
        <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-xl text-center">
          ...
          <Link href="/jobs" className="mt-4 inline-block text-sm text-emerald-400 hover:text-emerald-350 underline">Back to Job Orders</Link>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="space-y-8 animate-fadeIn">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">Job Order Details: {job.order_number}</h1>
          </div>
          <span className={`border rounded-full px-3 py-1 text-sm font-bold uppercase tracking-wider self-start sm:self-auto ${getStatusBadge(job.status)}`}>{job.status}</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="p-6 bg-zinc-900 border border-zinc-850 rounded-xl space-y-2">
            ...
            <div className="text-sm text-zinc-400">{job.status === "COMPLETED" ? `Actual Yield: ${job.actual_yield} units` : "Awaiting final batch run"}</div>
          </div>
          <div className="p-6 bg-zinc-900 border border-zinc-850 rounded-xl space-y-2">
            <div className="text-xs text-zinc-500 uppercase tracking-wider font-semibold">Calculated Cost Per Unit</div>
            <div className="text-lg font-bold text-emerald-400">{job.status === "COMPLETED" && job.actual_yield > 0 ? `$${(totalMaterialsCost / job.actual_yield).toFixed(2)}` : "$0.00"}</div>
            <div className="text-sm text-zinc-400">Total consumed material value: ${totalMaterialsCost.toFixed(2)}</div>
          </div>
        </div>
        <div className="bg-zinc-900 border border-zinc-850 rounded-xl p-6">
          ...
          {allocations.length === 0 ? (
            <div className="text-center py-12 text-zinc-500 text-sm border border-dashed border-zinc-800 rounded-lg">No materials allocated to this Job Order yet. Use the Store Keeper &quot;Issue to WIP&quot; panel.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="border-b border-zinc-800 text-zinc-400 font-semibold">
                    <th className="pb-3">SKU</th><th className="pb-3">Material Name</th><th className="pb-3">Batch Reference</th>
                    <th className="pb-3 text-right">Quantity Consumed</th><th className="pb-3 text-right">Unit Cost</th><th className="pb-3 text-right">Total Cost</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-800 text-zinc-350">
                  {allocations.map((a, i) => (
                    <tr key={i} className="hover:bg-zinc-850/10">
                      ...
                      <td className="py-3 text-right font-semibold text-emerald-400">${a.total_cost.toFixed(2)}</td>
                    </tr>
                  ))}
