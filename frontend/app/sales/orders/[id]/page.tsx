...
export default function SODetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchSO = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/v1/sales/orders/${resolvedParams.id}`, { headers });
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/v1/sales/orders/${resolvedParams.id}/confirm`, {
        method: "PATCH",
        headers,
      });
      if (res.ok) fetchSO();
        ...
        alert(err.detail || "Failed to confirm order");
      }
    } finally {
  ...
  const handleCreateInvoice = async () => {
    setActionLoading(true);
    try {
      const res = await fetch(`${API}/api/v1/sales/orders/${resolvedParams.id}/invoices?invoice_date=${new Date().toISOString().split("T")[0]}`, {
        method: "POST",
        headers,
      });
      if (res.ok) {
          ...
          <div className="flex gap-3">
            ...
            {so.status !== "DRAFT" && so.status !== "CANCELLED" && (
              <button
                ...
                disabled={actionLoading}
                className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white text-sm font-medium rounded-lg transition-all"
              >
                Generate Invoice
              </button>
            )}
